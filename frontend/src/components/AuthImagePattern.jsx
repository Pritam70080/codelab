...
import { CodeXml, Terminal, FileCode, Braces, Code } from "lucide-react"
...
function reverseList(head) {
  ...
  return (
    <div className="hidden lg:flex flex-col items-center justify-center text-white p-12 relative overflow-hidden">
      ...
      <div className="z-10 w-md flex flex-col items-center">
        {/* Code editor mockup */}
        <div className="w-full bg-slate-800 rounded-lg shadow-xl mb-8 overflow-hidden">
          ...
          <div className="p-4 font-mono text-xs sm:text-sm overflow-hidden relative h-64">
            ...
            {/* Blinking cursor */}
            <div className="absolute bottom-4 right-4 w-0.5 h-4 bg-white animate-blink"></div>
          </div>
        </div>
        ...
        {/* Text content */}
        <h2 className="text-2xl font-bold mb-4 text-center text-base-content">{title}</h2>
        <p className="text-base-content/80 text-center">{subtitle}</p>
      </div>
    </div>
  )
}
